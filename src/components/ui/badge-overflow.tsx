...
type BadgeOverflowElement = React.ComponentRef<"div">
...
type BadgeOverflowProps<T = string> = React.ComponentProps<"div"> &
  (T extends object ? GetBadgeLabel<T> : Partial<GetBadgeLabel<T>>) & {
    ...
    renderOverflow?: (count: number, hiddenItems: T[]) => React.ReactNode
    asChild?: boolean
  }
...
function BadgeOverflow<T = string>(props: BadgeOverflowProps<T>) {
  ...
  React.useLayoutEffect(() => {
    ...
    function measureContainer() {
      ...
      if (firstBadge) setBadgeHeight(firstBadge.offsetHeight || 20)

      const overflowChild = measureChildren[items.length] as HTMLElement | undefined
      if (overflowChild) setOverflowBadgeWidth(overflowChild.offsetWidth || 40)
  ...
  const { visibleItems, hiddenItems } = React.useMemo(() => {
    ...
    return {
      ...
      hiddenItems: items.slice(visible.length),
    }
    ...
    <>
      ...
      {isMeasured ? (
        ...
        <Comp
          ...
          ref={composedRef}
          className={cn("flex flex-wrap", className)}
          style={{ gap: badgeGap, minHeight: placeholderHeight, ...style }}
        >
          {items
